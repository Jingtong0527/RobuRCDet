...
DATA_PATH = 'your/path/nuscenes'
DATA_PATH1 = 'your/path/nuscenes'
RADAR_SPLIT = 'radar_bev_filter_id'
OUT_PATH = 'radar_pv_filter_id'
info_paths = ['your/path/nuscenes_infos_train.pkl', 'your/path/nuscenes_infos_val.pkl']
...
def worker(info):
    ...
    points = np.fromfile(os.path.join(DATA_PATH1, RADAR_SPLIT, radar_file_name),
                         dtype=np.float32,
                         count=-1).reshape(-1, 8)
    ...
    for i, cam_key in enumerate(cam_keys):
        ...
        pts_img, features_img,ids = map_pointcloud_to_image(
            pc.points.copy(), features.copy(),id_.copy(), IMG_SHAPE, cam_calibrated_sensor, cam_ego_pose)

        # print(features_img.shape)
        file_name = os.path.split(info['cam_infos'][cam_key]['filename'])[-1]
        np.concatenate([pts_img[:2, :].T, features_img,ids],
                       axis=1).astype(np.float32).flatten().tofile(
                           os.path.join(DATA_PATH1, OUT_PATH,
                                        f'{file_name}.bin'))
