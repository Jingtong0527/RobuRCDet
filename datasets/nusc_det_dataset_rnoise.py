...
class NuscDatasetRadarDet(Dataset):
    def __init__(self,
                 ...
                 radar_noise=0,# 0: clean, 1: key point missing, 2: spurious points, 3: point shifting, 4: noise disturbance.
                 camera_noise = 0, 
                 remove_z_axis=False,
                 use_cbgs=False,
    ...
    def transform_radar_pv(self, points, resize, resize_dims, crop, flip, rotate, radar_idx):
        ...
        # print(points.shape)
        # mask = torch.bernoulli(torch.tensor([0.2]))
        # rd_num = torch.randint(1,50,size=(1,)).item()
        # sigma = torch.where(mask==0,torch.tensor(0),torch.tensor(rd_num))
        
        rate= 0.2
        fixed_sigma = 5
        nn=1

        if self.radar_noise!=0:
            mask = torch.bernoulli(torch.tensor([rate]))

            if self.radar_noise==1: #key point missing
                ob_id = points[:, -1]
                if len(ob_id)>8:
                # print(ob_id)
                    to_drop = np.random.choice(ob_id, 8)
                    for id_ in to_drop:
                        points_to_drop = ob_id == id_

                        points = np.delete(points, points_to_drop, axis=0)
                        ob_id = np.delete(ob_id, points_to_drop, axis=0)
                

            elif self.radar_noise==2: #spurious point
                # print('2')
                sigma=fixed_sigma
                # sigma = torch.where(mask==0,torch.tensor(0),torch.tensor(fixed_sigma)) 
                std_dev = torch.tensor(sigma)
                to_add = points[:, 3:]
                points1 = points.clone()
                noise = torch.normal(mean=points1[:, :3], std=std_dev)
                noise = torch.cat((noise, to_add), dim=1)
                points = torch.vstack([noise,points])
                nn=2
            elif self.radar_noise==3: #point shifting
                sigma=fixed_sigma
                # sigma = torch.where(mask==0,torch.tensor(0),torch.tensor(fixed_sigma)) 
                std_dev = torch.tensor(sigma)
                to_add = points[:, 3:]
            ...
            elif self.radar_noise==4: #non-positional disturbance
                sigma=fixed_sigma
                # sigma = torch.where(mask==0,torch.tensor(0),torch.tensor(fixed_sigma)) 
                std_dev = torch.tensor(sigma)
                to_add1 = points[:, 6:]
                to_add2 = points[:, :3]
                points1 = points.clone()
                noise = torch.normal(mean=points1[:, 3:6], std=std_dev)
                points = torch.cat((to_add2, noise, to_add1), dim=1)
        ...
        for cam in cams:
            ...
            for sweep_idx, cam_info in enumerate(cam_infos):
                ...
                if self.return_radar_pv:
                    ...
                    ### here
                    # print(radar_point[:,:2])
                    # random_noise = np.random.rand(radar_point.shape[0],2) * 2 - 1
                    # print(random_noise)
                    # radar_point[:, :2] += random_noise
